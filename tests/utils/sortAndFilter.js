...
module.exports = _module = {
    ...
    filter_switch: function (item, filterId, filter) {
        ...
        if (filter instanceof Array) {
            // if filter is an array of objects to be compared,
            // loop through each and reduce

            // if (filter[0].op && compare[filter[0].op]) {
            //     console.log('IN FILTER OP, filterID', filterId);
            //     console.log('IN FILTER OP', filter)
            //     return filter.reduce(function (prev, filterObj) {
        ...
        //     switch (filter.op) {
        //         case "number":
        //             return Number(item[field.id]) === Number(search);
        //         case "boolean":
        //             if (search.toLowerCase() === 'false' || Number(search) === 0) {
        //                 return !item[field.id];
        //             } else if (search.toLowerCase() === 'true' || Number(search) === 1) {
        //                 return !!item[field.id];
        //             } else {
        //                 return false;
        //             }
        //         case "datetime":
        //             try {
        //                 return new Date(item[field.id]).toISOString()
        //                     .indexOf(search) >= 0;
        //             } catch (ex) {
        //                 return false;
        //             }
        //         case "string_match":
        //             return String(item[field.id]) === search;
        //         default:
        //             if(search instanceof Array)
        //             {
        //                // see if value exists in array
        //                return ~search.indexOf(item[field.id]);
        //             }
        //             return String(item[field.id]).indexOf(search) === 0;
        //     }
    },
